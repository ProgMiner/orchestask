...
func Init(basePath string) (*BaseStorage, error) {
	s := &BaseStorage{basePath: filepath.Clean(basePath)}
...
func (s *BaseStorage) makeLockFile() error {
...
func (storage *BaseStorage) ensureDir(dir string) (string, error) {
	path := filepath.Join(storage.basePath, dir)
	return path, os.MkdirAll(path, 0755)
}

func (storage *BaseStorage) initIndexes(dir string, handler func(id model.ID) error) error {
...
func (storage *BaseStorage) readFile(dir string, id model.ID, result any) error {
...
func (storage *BaseStorage) saveFile(dir string, id model.ID, create createFile, data any) error {
	path := filepath.Join(storage.basePath, dir, string(id))
